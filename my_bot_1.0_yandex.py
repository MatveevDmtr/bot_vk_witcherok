...
def photos(vk_session, user_id):
    a = vk_session.method("photos.getMessages.UploadServer")
    b = requests.post(a['upload_url'], files={'photo': open('sch.jpg', 'rb')}).json()
    c = vk_session.method('photos.saveMessagesPhoto', {'photo': b['photo'], 'server': b['server'], 'hash': b['hash']})[0]
    d = "photo{}_{}".format(c["owner_id"], c["id"])
    vk_session.method('messages.send', {'peer_id': user_id, 'attachment': d})
